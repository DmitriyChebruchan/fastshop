...
from src.catalogue.models.database import Product, AdditionalProducts, \
    RecommendedProducts
...
class AdditionalProductsRepository(
    BaseSQLAlchemyRepository[AdditionalProducts]):
    def __init__(self, session: AsyncSession):
        super().__init__(model=AdditionalProducts, session=session)


class RecommendedProductsRepository(
    BaseSQLAlchemyRepository[RecommendedProducts]):
    def __init__(self, session: AsyncSession):
        super().__init__(model=RecommendedProducts, session=session)


def get_additional_products_repository(session: AsyncSession = Depends(
    get_session)) -> AdditionalProductsRepository:
    return AdditionalProductsRepository(session=session)


def get_recommended_products_repository(session: AsyncSession = Depends(
    get_session)) -> RecommendedProductsRepository:
